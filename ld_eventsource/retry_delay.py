...
class _RetryDelayStrategy:
    ...
    def __init__(
        ...
        self.__good_since = None  # type: Optional[float]

    def next_retry_delay(self, current_time: float) -> float:
    ...
    def set_good_since(self, good_since: float):
        """Marks the current state as "good" and records the time.

        :param good_since: the time that the state became "good", in seconds
        """
...
class _DefaultBackoffStrategy:
    ...
    """
    def __init__(self, max_delay: Optional[float] = None):
        self.__max_delay = max_delay

    def apply_backoff(self, delay: float, retry_count: int) -> float:
        d = delay * (2 ** retry_count)
        return d if (self.__max_delay is None or d <= self.__max_delay) else self.__max_delay

class _DefaultJitterStrategy:
    """The default implementation of jitter, which subtracts a pseudo-random amount from each delay.
    """
    def __init__(self, ratio: float, rand_seed = None):
        """Creates an instance.
    ...
    def apply_jitter(self, delay: float):
